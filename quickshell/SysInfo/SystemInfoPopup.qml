...
PanelWindow {
    ...
    Connections {
        ...
        function onCpuPctChanged() {
            sp.cpuHist = sp.pushH(sp.cpuHist, SystemStats.cpuPct);
        }
        function onCpuTempChanged() {
            sp.tempHist = sp.pushH(sp.tempHist, SystemStats.cpuTemp);
        }
        function onRamPctChanged() {
            sp.ramHist = sp.pushH(sp.ramHist, SystemStats.ramPct);
        }
    }
        ...
        onRunningChanged: {
            if (!running) {
                ...
                if (diskProc.stdout._tmp.length === 0) {
                    diskProc2.running = false;
                    diskProc2.running = true;
                } else {
    ...
    Process {
        id: osProc
        command: ["sh", "-c", "grep PRETTY_NAME /etc/os-release | sed 's/PRETTY_NAME=//;s/\"//g;s/—//g;s/ - //g'"]
        stdout: SplitParser {
            onRead: function(d) {
                sp.osName = d.trim().replace(/[\u2014\u2013\u2012]/g, "").trim();
            }
        }
        Component.onCompleted: running = true
    }
    ...
    ColumnLayout {
                ...
                Item {
                    ...
                    Rectangle {
                        ...
                        Image {
                            ...
                            height: 16
                            smooth: true
                            source: sp.ico(sp.i_cpu, "%23ebdbb2", 16)
                            layer.enabled: true
                    ...
                    Canvas {
                        ...
                        height: 40
                        property var hist: sp.cpuHist

                        onHistChanged: requestPaint()
                        onPaint: {
                            var c = getContext("2d");
                            c.clearRect(0, 0, width, height);
                            c.strokeStyle = Colors.primary//"#83a598";
                            c.lineWidth = 1.8;
                            c.lineJoin = "round";
                            c.beginPath();
                            for (var i = 0; i < hist.length; i++) {
                                var px = i / (hist.length - 1) * width;
                                var py = height - (hist[i] / 100) * height * .85;
                                i ? c.lineTo(px, py) : c.moveTo(px, py);
                            }
                            c.stroke();
                            c.fillStyle = Colors.primary//"#83a598";
                            c.beginPath();
                            c.arc(width, height - (hist[hist.length - 1] / 100) * height * .85, 3, 0, Math.PI * 2);
                            c.fill();
                        }
                    }
                }
                ...
                Item {
                    ...
                    Rectangle {
                        ...
                        Image {
                            ...
                            height: 16
                            smooth: true
                            source: sp.ico(sp.i_therm, "%23ebdbb2", 16)
                            layer.enabled: true
                            layer.effect: ColorOverlay {
                                color: Colors.surfacefg
                            }
                    ...
                    Canvas {
                        ...
                        width: parent.width - 88
                        height: 40
                        property var hist: sp.tempHist

                        onHistChanged: requestPaint()
                        onPaint: {
                            var c = getContext("2d");
                            c.clearRect(0, 0, width, height);
                            c.strokeStyle = Colors.tertiary//"#fabd2f";
                            c.lineWidth = 1.8;
                            c.lineJoin = "round";
                            c.beginPath();
                            for (var i = 0; i < hist.length; i++) {
                            ...
                            c.stroke();
                            c.fillStyle = Colors.tertiary//"#fabd2f";
                            c.beginPath();
                            c.arc(width, height - (hist[hist.length - 1] / 100) * height * .85, 3, 0, Math.PI * 2);
                            c.fill();
                        }
                ...
                Item {
                    ...
                    Rectangle {
                        ...
                        Image {
                            ...
                            height: 16
                            smooth: true
                            source: sp.ico(sp.i_ram, "%23ebdbb2", 16)
                            layer.enabled: true
                            layer.effect: ColorOverlay {
                                color: Colors.surfacefg
                            }
                    ...
                    Canvas {
                        ...
                        width: parent.width - 88
                        height: 40
                        property var hist: sp.ramHist

                        onHistChanged: requestPaint()
                        onPaint: {
                            var c = getContext("2d");
                            c.clearRect(0, 0, width, height);
                            c.strokeStyle = Colors.secondary//"#d3869b";
                            c.lineWidth = 1.8;
                            c.lineJoin = "round";
                            c.beginPath();
                            for (var i = 0; i < hist.length; i++) {
                            ...
                            c.stroke();
                            c.fillStyle = Colors.secondary//"#d3869b";
                            c.beginPath();
                            c.arc(width, height - (hist[hist.length - 1] / 100) * height * .85, 3, 0, Math.PI * 2);
                            c.fill();
                        }
                ...
                Column {
                    ...
                    Row {
                        ...
                        Image {
                            ...
                            smooth: true
                            anchors.verticalCenter: parent.verticalCenter
                            source: sp.ico(sp.i_down, "%238ec07c", 14)
                        }
                        Text {
                            text: sp.fmtNet(sp.rxKb)
                            color: Colors.surfacefg
                            font.family: mainFont
                            font.pixelSize: 16
                            font.weight: Font.Medium
                ...
                Column {
                    ...
                    Row {
                        ...
                        Image {
                            ...
                            smooth: true
                            anchors.verticalCenter: parent.verticalCenter
                            source: sp.ico(sp.i_up, "%23fe8019", 14)
                        }
                        Text {
                            text: sp.fmtNet(sp.txKb)
                            color: Colors.surfacefg
                            font.family: mainFont
                            font.pixelSize: 16
                            font.weight: Font.Medium
        ...
        Repeater {
            model: sp.diskList.length > 0 ? sp.diskList : [{
                mount: "/",
                total: "",
                used: "",
                free: "",
        ...
        Rectangle {
            ...
            Item {
                ...
                Column {
                    ...
                    Text {
                        text: sp.hostname || "No HostName Provided"
                        color: Colors.surfacefg
                        font.family: mainFont
                        font.pixelSize: 16
                        font.weight: Font.Medium
