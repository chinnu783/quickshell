...
PanelWindow {
    ...
    NotificationServer {
        ...
        onNotification: notif => {
            var newArr = cal.notifList.slice()
            newArr.unshift(notif)
            cal.notifList = newArr
        }
    }

    function removeNotification(notifObj) {
        if (notifObj && typeof notifObj.dismiss === "function") {
            try { notifObj.dismiss(); } catch(e) {}
        }
        var newArr = cal.notifList.filter(item => item !== notifObj)
        cal.notifList = newArr
    }

    function clearAllNotifications() {
        for (var i = 0; i < cal.notifList.length; i++) {
            var item = cal.notifList[i]
        ...
        Rectangle {
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    Rectangle {
                        visible: cal.notifList.length > 0
                                    ...
                                    RowLayout {
                                        ...
                                        Rectangle {
                                            ...
                                            MouseArea {
                                                ...
                                                hoverEnabled: true
                                                cursorShape: Qt.PointingHandCursor
                                                onClicked: cal.removeNotification(modelData)
                                            }
                                        }
                                    }
