...
PanelWindow {
    ...
    FileView {
        id: notifFile
        // Resolves path relative to this QML file inside ~/.config/quickshell
        path: Qt.resolvedUrl("./notifications_cache.json").toString().replace("file://", "")
        blockLoading: true

        onLoaded: {
            var rawText = notifFile.text()
            if (!rawText || rawText.trim() === "") return;
            try {
                var list = JSON.parse(rawText)
                calNotifModel.clear()
                for (var i = 0; i < list.length; i++) {
                    calNotifModel.append(list[i])
                }
            } catch (e) {
                console.log("Failed to parse notifications file:", e)
            }
        }
    }

    Component.onCompleted: {
        // Load stored notifications on startup if FileView is ready
        if (notifFile.loaded) {
            var rawText = notifFile.text()
    ...
    function saveNotifications() {
        var items = []
        for (var i = 0; i < calNotifModel.count; i++) {
            var item = calNotifModel.get(i)
            items.push({
                "notifId": item.notifId || 0,
                "appName": item.appName || "",
                "summary": item.summary || "",
                "body": item.body || ""
            })
        }
        notifFile.setText(JSON.stringify(items, null, 2))
    }
    ...
    NotificationServer {
        ...
        onNotification: notif => {
            ...
            cal.saveNotifications()
        }
    }

    function removeNotification(index) {
        if (index >= 0 && index < calNotifModel.count) {
            var item = calNotifModel.get(index)
            if (item && item.obj && typeof item.obj.dismiss === "function") {
                try { item.obj.dismiss(); } catch(e) {}
            }
            calNotifModel.remove(index)
            cal.saveNotifications()
        }
    }
                ...
                RowLayout {
                    ...
                    Rectangle {
                        ...
                        MouseArea {
                            ...
                            onClicked: cal.clearAllNotifications()
                        }
                    }
                }
                            ...
                            delegate: Rectangle {
                                ...
                                ColumnLayout {
                                    ...
                                    RowLayout {
                                        ...
                                        Text {
                                            Layout.fillWidth: true
                                            text: notifCard.appName ? notifCard.appName : "System"
                                            color: Colors.outline
                                            font.family: mainFont
                                            font.pixelSize: 11
                                            font.weight: Font.DemiBold
                                        ...
                                        Rectangle {
                                            ...
                                            MouseArea {
                                                ...
                                                hoverEnabled: true
                                                cursorShape: Qt.PointingHandCursor
                                                onClicked: cal.removeNotification(notifCard.index)
                                            }
                                        }
                                    }
                                    ...
                                    Text {
                                        Layout.fillWidth: true
                                        text: notifCard.summary ? notifCard.summary : ""
                                        visible: text !== ""
                                        color: Colors.surfacefg
                                        font.family: mainFont
                                        font.pixelSize: 12
                                    ...
                                    Text {
                                        Layout.fillWidth: true
                                        text: notifCard.body ? notifCard.body : ""
                                        visible: text !== ""
                                        color: Colors.surfacefg
                                        opacity: 0.8
                                        font.family: mainFont
