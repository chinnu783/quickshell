...
PanelWindow {
    ...
    // FIX 1: Restrict mask ONLY to the visual bgPanel to prevent empty padding input grabs
    mask: Region {
        item: launcher.isOpen ? bgPanel : null
    }
    ...
    // FIX 2: Propagate events so scrolling/clicking outside works properly
    MouseArea {
        anchors.fill: parent
        propagateComposedEvents: true
        onClicked: (mouse) => {
            launcher.isOpen = false;
            mouse.accepted = false; // Lets input pass to lower windows
        }
        onWheel: (wheel) => {
            wheel.accepted = false; // Prevents trapping global scroll wheel events
        }
    }
    ...
    Item {
        ...
        Canvas {
            ...
            width: 24
            height: 24
            renderTarget: Canvas.Image // FIX 3: Caches drawing to prevent redraw stutter
