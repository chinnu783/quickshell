...
PanelWindow {
    ...
    Connections {
        ...
        function onCpuPctChanged() {
            cc.cpuHistory = cc.pushHistory(cc.cpuHistory, SystemStats.cpuPct);
        }
        function onCpuTempChanged() {
            cc.tempHistory = cc.pushHistory(cc.tempHistory, SystemStats.cpuTemp);
        }
        function onRamPctChanged() {
            cc.ramHistory = cc.pushHistory(cc.ramHistory, SystemStats.ramPct);
        }
    }
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: function(d) {
                ...
                var m = d.match(/([\d.]+)/);
                if (m)
                    cc.volPct = Math.round(parseFloat(m[1]) * 100);
            }
        }
    }

    function setVol(v) {
        cc.volPct = Math.max(0, Math.min(150, v));
        Qt.createQmlObject('import Quickshell.Io; Process{command:["wpctl","set-volume","@DEFAULT_SINK@","' + (cc.volPct / 100).toFixed(2) + '"]; running:true}', cc, "sv");
    }

    function toggleMute() {
        cc.volMute = !cc.volMute;
        Qt.createQmlObject('import Quickshell.Io; Process{command:["wpctl","set-mute","@DEFAULT_SINK@","toggle"]; running:true}', cc, "tm");
    }

    property int _bc: 0
    property int _bm: 255

    Process {
        id: briProc
        command: ["sh", "-c", "echo $(brightnessctl get) && echo $(brightnessctl max)"]
        ...
        stdout: SplitParser {
            ...
            onRead: function(d) {
                if (_l === 0) {
                    cc._bc = parseInt(d.trim()) || 0;
                    _l = 1;
                } else {
                    cc._bm = parseInt(d.trim()) || 255;
                    cc.briPct = Math.round(cc._bc / cc._bm * 100);
                    _l = 0;
                }
            }
            Component.onCompleted: _l = 0
    ...
    function setBri(v) {
        cc.briPct = Math.max(1, Math.min(100, v));
        Qt.createQmlObject('import Quickshell.Io; Process{command:["brightnessctl","set","' + cc.briPct + '%"]; running:true}', cc, "sb");
    }

    Process {
        id: wifiProc
        command: ["sh", "-c", "nmcli radio wifi; nmcli -t -f SSID,SIGNAL,ACTIVE dev wifi list 2>/dev/null | head -8"]
        ...
        cc.wifiNetworks = wifiProc.stdout._n ? wifiProc.stdout._n.slice() : []
    }

    function toggleWifi() {
        cc.wifiOn = !cc.wifiOn;
        Qt.createQmlObject('import Quickshell.Io; Process{command:["nmcli","radio","wifi","' + (cc.wifiOn ? "on" : "off") + '"]; running:true}', cc, "tw");
        if (!cc.wifiOn) {
            cc.wifiName = "";
            cc.wifiNetworks = [];
        }
    ...
    function connectWifi(ssid) {
        Qt.createQmlObject('import Quickshell.Io; Process{command:["sh","-c","nmcli con up \\"' + ssid + '\\" 2>/dev/null || nmcli dev wifi connect \\"' + ssid + '\\""]; running:true}', cc, "cw");
        cc.wifiName = ssid;
        cc.wifiOpen = false;
    }
    ...
    Process {
        id: btProc
        command: ["sh", "-c", "bluetoothctl show | grep Powered | awk '{print $2}'; bluetoothctl devices Connected 2>/dev/null"]
        ...
        onRunningChanged: {
            if (!running) {
                cc.btDevices = btProc.stdout._d ? btProc.stdout._d.slice() : [];
                if (cc.btDevices.length > 0)
                    cc.btDevice = cc.btDevices[0].name;
            }
        }
    }
    ...
    function toggleBt() {
        cc.btOn = !cc.btOn;
        Qt.createQmlObject('import Quickshell.Io; Process{command:["sh","-c","bluetoothctl power ' + (cc.btOn ? "on" : "off") + '"]; running:true}', cc, "tb");
        if (!cc.btOn) {
            cc.btDevice = "";
            cc.btDevices = [];
        }
    ...
    function disconnectBt(mac) {
        Qt.createQmlObject('import Quickshell.Io; Process{command:["bluetoothctl","disconnect","' + mac + '"]; running:true}', cc, "db");
        cc.btDevice = "";
        cc.btOpen = false;
        btProc.running = false;
        btProc.running = true;
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: function(d) {
                cc.powerProfile = d.trim();
            }
        }
    }

    function cyclePP() {
        var p = ["power-saver", "balanced", "performance"];
        var n = p[(p.indexOf(cc.powerProfile) + 1) % 3];
        cc.powerProfile = n;
        Qt.createQmlObject('import Quickshell.Io; Process{command:["powerprofilesctl","set","' + n + '"]; running:true}', cc, "pp");
    }
    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            ColumnLayout {
                ...
                Rectangle {
                    ...
                    height: 52
                    radius: 8
                    color: cc.wifiOn ? Colors.primary : Colors.module
                    ...
                    RowLayout {
                        ...
                        Rectangle {
                            ...
                            height: 36
                            radius: 7
                            color: cc.wifiOn ? Colors.primaryfg : Colors.module_hover
                            ...
                            Image {
                                ...
                                height: 22
                                smooth: true
                                source: cc.ico(cc.p_wifi, cc.wifiOn ? Colors.primary : Colors.outline, 22)
                            }
                        }
                        ...
                        ColumnLayout {
                            ...
                            Text {
                                text: "WiFi"
                                color: cc.wifiOn ? Colors.primaryfg : Colors.surfacefg
                                font.family: "Google Sans"
                                font.pixelSize: 14
                                font.weight: Font.Medium
                            ...
                            Text {
                                visible: cc.wifiOn && cc.wifiName !== ""
                                text: cc.wifiName
                                color: cc.wifiOn ? Colors.primaryfg : Colors.outline
                                font.family: "Google Sans"
                                font.pixelSize: 12
                ...
                Rectangle {
                    ...
                    height: 52
                    radius: 8
                    color: cc.btOn ? Colors.primary : Colors.module
                    ...
                    RowLayout {
                        ...
                        Rectangle {
                            ...
                            height: 36
                            radius: 7
                            color: cc.btOn ? Colors.primaryfg : Colors.module_hover
                            ...
                            Image {
                                ...
                                height: 22
                                smooth: true
                                source: cc.ico(cc.p_bt, cc.btOn ? Colors.primary : Colors.outline, 22)
                            }
                        }
                        ...
                        ColumnLayout {
                            ...
                            Text {
                                text: "Bluetooth"
                                color: cc.btOn ? Colors.primaryfg : Colors.surfacefg
                                font.family: "Google Sans"
                                font.pixelSize: 14
                                font.weight: Font.Medium
                                horizontalAlignment: Text.AlignHCenter
                            ...
                            Text {
                                visible: cc.btOn && cc.btDevice !== ""
                                text: cc.btDevice
                                color: cc.btOn ? Colors.primaryfg : Colors.outline
                                font.family: "Google Sans"
                                font.pixelSize: 12
                                elide: Text.ElideRight
                                Layout.fillWidth: true
            ...
            Item {
                ...
                Rectangle {
                    ...
                    Rectangle {
                        ...
                        radius: 10
                        color: cc.volMute ? Colors.outline : Colors.primary
                    ...
                    Image {
                        anchors.horizontalCenter: parent.horizontalCenter
                        anchors.bottom: volFill.bottom
                        anchors.bottomMargin: 8
                        width: 18
                        height: 18
                        smooth: true
                        source: cc.ico(cc.volMute ? cc.p_volX : cc.volPct <= 30 ? cc.p_vol1 : cc.volPct <= 60 ? cc.p_vol2 : cc.p_vol3, cc.volMute ? Colors.surfacefg : Colors.primaryfg, 18)
                    }
                }

                MouseArea {
                    anchors.fill: volTrack
                    cursorShape: Qt.PointingHandCursor
                    hoverEnabled: true

                    onWheel: function(w) {
                        cc.setVol(cc.volPct + (w.angleDelta.y > 0 ? 5 : -5));
                    }
                    onPressed: function(m) {
                        var p = Math.round((1 - m.y / volTrack.height) * 100);
                        cc.setVol(Math.max(0, Math.min(100, p)));
                    }
            ...
            Item {
                ...
                Rectangle {
                    ...
                    Image {
                        ...
                        height: 18
                        smooth: true
                        source: cc.ico(cc.briPct === 0 ? cc.p_briOff : cc.p_bri, Colors.primaryfg, 18)
                    }
                }

                MouseArea {
                    anchors.fill: briTrack
                    cursorShape: Qt.PointingHandCursor
                    hoverEnabled: true

                    onWheel: function(w) {
                        cc.setBri(cc.briPct + (w.angleDelta.y > 0 ? 5 : -5));
                    }
                    onPressed: function(m) {
                        var p = Math.round((1 - m.y / briTrack.height) * 100);
                        cc.setBri(Math.max(1, Math.min(100, p)));
                    }
                    onPositionChanged: function(m) {
                        if (!pressed)
                            return;
        ...
        // Column {
        //     Layout.fillWidth: true
        //     visible: cc.wifiOpen && cc.wifiNetworks.length > 0
        //     spacing: 1
        //
        //     Repeater {
        ...
        //             height: 34
        //             radius: 6
        //             color: wMa.containsMouse ? Colors.module_hover : "transparent"
        //
        //             Behavior on color {
        //                 ColorAnimation {
        //                     duration: 60
        ...
        //                             height: 4 + index * 3
        //                             anchors.bottom: parent.bottom
        //                             color: modelData.signal >= (index + 1) * 25 ? Colors.primary : Colors.outline
        //                         }
        //                     }
        //                 }
        //
        ...
        //                     height: 13
        //                     smooth: true
        //                     source: cc.ico(cc.p_check, Colors.primary, 13)
        //                 }
        //             }
        //
        //             MouseArea {
        ...
        //                     height: 15
        //                     smooth: true
        //                     source: cc.ico(cc.p_bt, Colors.primary, 15)
        //                 }
        //
        //                 Text {
        //                     text: modelData.name
        ...
        //                         anchors.fill: parent
        //                         cursorShape: Qt.PointingHandCursor
        //                         onClicked: cc.disconnectBt(modelData.mac)
        //                     }
        //                 }
        //             }
        //         }
        ...
        Rectangle {
            ...
            RowLayout {
                ...
                Image {
                    ...
                    height: 26
                    smooth: true
                    source: cc.ico(cc.p_zap, cc.powerProfile === "performance" ? Colors.tertiary : cc.powerProfile === "power-saver" ? Colors.secondary : Colors.outline, 26)
                }

                Text {
                    text: cc.powerProfile === "performance" ? "Performance" : cc.powerProfile === "power-saver" ? "Power Saver" : "Balanced"
                    color: cc.powerProfile === "performance" ? Colors.tertiary : cc.powerProfile === "power-saver" ? Colors.secondary : Colors.surfacefg
                    font.family: "Google Sans"
                    font.pixelSize: 15
                    font.weight: Font.Medium
                    Layout.fillWidth: true
                ...
                Image {
                    ...
                    height: 14
                    smooth: true
                    source: cc.ico(cc.p_chevR, Colors.outline, 14)
                }
            }
